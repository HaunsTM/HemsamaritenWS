namespace Tellstick.TelldusNETWrapper
{
    using System;
    ...
    public sealed class TelldusNETWrapper	//don't allow inheritance
	{
		...
		{
			//clean up
			if (this.registeredEventFunctionId != -1)
			{
				UnmanagedImport.tdUnregisterCallback(this.registeredEventFunctionId);
			}
			if (this.registeredDeviceChangeEventFunctionId != -1)
			{
				UnmanagedImport.tdUnregisterCallback(this.registeredDeviceChangeEventFunctionId);
			}
			if (this.registeredRawListenerFunctionId != -1)
			{
				UnmanagedImport.tdUnregisterCallback(this.registeredRawListenerFunctionId);
			}
        ...
        /// <summary>
		/// [Async] Add a new device to the global database of devices. This function must be called first before
		/// any call to tdSetName(), tdSetProtocol() and similar functions.
 		/// </summary>
		/// <returns> The device id for the newly created device. If the creation fails it returnes a 
		/// negative value.</returns>
        static Task<int> tdAddDeviceAsync()
        {
            return Task.Run<int>(() =>
            {
                return tdAddDevice();
        ...
        /// <summary>
        ///  [Async] Sends bell command to devices supporting this.
        ///  Make sure the device supports this by calling tdMethods() before any calls to this function.
        /// </summary>
        /// <param name="deviceId">Id of device to sound the bell on</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdBellAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
        ...
        /// <summary>
        ///   [Async] Close the library and clean up the cache it uses.
        ///   This should be called when the library is not supposed to be used anymore.
        ///   Do not use when this has been instantiated, will be closed in destructor then, only on static methods.
        /// </summary>
        public static Task tdCloseAsync()
        {
            return Task.Run(() =>
            {
                tdClose();
        ...
        /// <param name="level">The level the device should dim to. This value should be 0-255</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdDimAsync(int deviceId, char level)
        {
            return Task.Run<int>(() =>
            {
        ...
        /// <summary>
        /// [Async] Send "down" command to device.
        /// Make sure the device supports this by calling tdMethods() before any calls to this function.
        /// </summary>
        /// <param name="deviceId">The device id to send the command to</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdDownAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return tdDown(deviceId);
            });
        }
        ...
        /// <summary>
        /// [Async] Execute a scene action.
        /// Make sure the device supports this by calling tdMethods() before any calls to this function.
        /// </summary>
        /// <param name="deviceId">Id of device to perform the execute action on</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdExecuteAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return tdExecute(deviceId);
            });
        }
        ...
        /// <summary>
        ///  [Async] This function returns the unique id of a device with a specific index.
        ///  To get all the id numbers you should loop over all the devices, use tdGetNumberOfDevices() to get the number of devices
        /// </summary>
        /// <param name="order">Device index</param>
        /// <returns>Id of device at that position</returns>
        public static Task<int> tdGetDeviceIdAsync(int order)
        {
            return Task.Run<int>(() =>
            {
                return tdGetDeviceId(order);
            });
        }
        ...
        /// <summary>
        /// [Async] Get a protocol specific device parameter
        /// </summary>
        /// <param name="deviceId">The id of the device to query</param>
        /// <param name="name">The name of the parameter to query</param>
        /// <param name="defaultValue">A defaultValue to return if the current parameter hasn't previously been set</param>
        /// <returns>Any protocol specific parameter specified by "name"</returns>
        public static unsafe Task<string> tdGetDeviceParameterAsync(int deviceId, string name, string defaultValue)
        {
        ...
        /// <summary>
        /// [Async] Returns which type the device is
        /// </summary>
        /// <param name="deviceId">Id of the device</param>
        /// <returns>TELLSTICK_TYPE_DEVICE or TELLSTICK_TYPE_GROUP</returns>
        public static Task<int> tdGetDeviceTypeAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdGetDeviceType(deviceId);
        ...
        /// <summary>
        /// [Async] Get a human readable string from an error code returned from a function in telldus-core
        /// </summary>
        /// <param name="errorNo">The error code to translate</param>
        /// <returns>A string ready to show to the user.
        /// TELLSTICK_SUCCESS
        ...
        /// TELLSTICK_ERROR_UNKNOWN
        /// </returns>
        public static unsafe Task<string> tdGetErrorStringAsync(int errorNo)
        {
        ...
        /// <summary>
        /// [Async] Query a device for it’s name
        /// </summary>
        /// <param name="deviceId">Id of device to query</param>
        /// <returns>The name of the device or an empty string if the device is not found</returns>
        public static unsafe Task<string> tdGetNameAsync(int deviceId)
        {
            return Task.Run<string>(() =>
            {
                return getString(UnmanagedImport.tdGetName(deviceId));
        ...
        /// <summary>
        ///  [Async] Query a device for which methods it supports. By supplying the methods you support 
        ///  the library could remap the methods a device support for better fit the application.
        /// </summary>
        /// <param name="deviceId">Id of device to query</param>
        /// <param name="methodsSupported">Methods that the client application supports</param>
        ...
        /// TELLSTICK_LEARN
        /// </returns>
        public static Task<int> tdMethodsAsync(int deviceId, int methodsSupported)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdMethods(deviceId, methodsSupported);
        ...
        /// <summary>
        /// [Async] Query a device for it’s model
        /// </summary>
        /// <param name="deviceId">Id of device to query</param>
        /// <returns>The model for a device. Not all protocols use this.</returns>
        public static unsafe Task<string> tdGetModelAsync(int deviceId)
        {
            return Task.Run<string>(() =>
            {
                return getString(UnmanagedImport.tdGetModel(deviceId));
        ...
        /// <summary>
        /// [Async] This function returns the number of devices configured
        /// </summary>
        /// <returns>The total number of devices configured</returns>
        public static Task<int> tdGetNumberOfDevicesAsync()
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdGetNumberOfDevices();
        ...
        /// <summary>
        /// [Async] Query a device for it’s protocol
        /// </summary>
        /// <param name="deviceId">Id of device to query</param>
        /// <returns>The protocol for a device.</returns>
        public static unsafe Task<string> tdGetProtocolAsync(int deviceId)
        {
            return Task.Run<string>(() =>
            {
                return getString(UnmanagedImport.tdGetProtocol(deviceId));
        ...
        /// <summary>
        /// [Async] Returns the last sent command to a specific device
        /// </summary>
        /// <param name="deviceId">Id of device to query</param>
        /// <param name="methods">The methods supported by the client. See tdMethods() for more information.</param>
        /// <returns>The last sent command as integer, for example TELLSTICK_TURNON or TELLSTICK_TURNOFF</returns>
        public static Task<int> tdLastSentCommandAsync(int deviceId, int methods)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdLastSentCommand(deviceId, methods);
        ...
        /// <summary>
        /// [Async] If the last sent command it TELLSTICK_DIM this returns the dimmed value
        /// </summary>
        /// <param name="deviceId">Id of device to query</param>
        /// <returns>The the value as a human readable string, for example "128" for 50%</returns>
        public static unsafe Task<string> tdLastSentValueAsync(int deviceId)
        {
            return Task.Run<string>(() =>
            {
                return getString(UnmanagedImport.tdLastSentValue(deviceId));
        ...
        /// <summary>
        /// [Async] Sends a special learn command to some devices that need a special learn-command to be used from TellStick
        /// Make sure the device supports this by calling tdMethods() before any calls to this function.
        /// </summary>
        /// <param name="deviceId">Id of device to learn</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdLearnAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdLearn(deviceId);
        ...
        public unsafe int tdRegisterDeviceEvent(EventCallbackFunction deviceEventFunc, Object obj)
		{
			...
			if (this.deviceEventList.Count == 0)
			{
				//first added, register with dll too
				UnmanagedImport.EventFunctionDelegate deviceEventFunctionDelegate = new UnmanagedImport.EventFunctionDelegate(this.deviceEventFunction);
				
				this.registeredEventFunctionId = UnmanagedImport.tdRegisterDeviceEvent(deviceEventFunctionDelegate, (void*)null);	//context here or above?
				GC.Collect();
				this.callbackFunctionReferenceList.Add(this.registeredEventFunctionId, deviceEventFunctionDelegate);
			}

			++this.lastEventID;
			returnValue = this.lastEventID;
			...
			this.deviceEventList.Add(returnValue, deviceEventFuncContext);
		...
		{
			int returnValue = 0;
			if (this.deviceChangeEventList.Count == 0)
			{
				//first added, register with dll too
				//only the context object of the first event will be registered
				UnmanagedImport.DeviceChangeEventFunctionDelegate deviceChangeEventFunctionDelegate = new UnmanagedImport.DeviceChangeEventFunctionDelegate(this.deviceChangeEventFunction);

				this.registeredDeviceChangeEventFunctionId = UnmanagedImport.tdRegisterDeviceChangeEvent(deviceChangeEventFunctionDelegate, (void*)null);
				GC.Collect();
				this.callbackFunctionReferenceList.Add(this.registeredDeviceChangeEventFunctionId, deviceChangeEventFunctionDelegate);
			}
			++this.lastEventID;
			returnValue = this.lastEventID;
			...
			this.deviceChangeEventList.Add(returnValue, deviceChangeEventFuncContext);
		...
		{
			int returnValue = 0;
			if (this.rawListenerList.Count == 0)
			{
				//first added, register with dll too
				//only the context object of the first event will be registered
				UnmanagedImport.RawListeningDelegate listeningFunctionDelegate = new UnmanagedImport.RawListeningDelegate(this.rawListeningFunction);

				this.registeredRawListenerFunctionId = UnmanagedImport.tdRegisterRawDeviceEvent(listeningFunctionDelegate, (void*)null);
				GC.Collect();
				this.callbackFunctionReferenceList.Add(this.registeredRawListenerFunctionId, listeningFunctionDelegate);
			}
			++this.lastEventID;
			returnValue = this.lastEventID;
			...
			this.rawListenerList.Add(returnValue, rawEventFuncContext);
			return returnValue;
		}
        ...
        /// <summary>
         /// [Async] Removes a device.
         /// </summary>
         /// <param name="deviceId">Id of device to remove</param>
         /// <returns>True on success, false otherwise</returns>
        public static Task<bool> tdRemoveDeviceAsync(int deviceId)
        {
        ...
        /// <summary>
         /// [Async] ]Send a raw command to TellStick. Please read the TellStick protocol definition on how the command should be constructed.
         /// </summary>
         /// <param name="command">The command for TellStick in its native format</param>
         /// <param name="reserved"></param>
         /// <returns>Success or error code</returns>
        public static unsafe Task<int> tdSendRawCommandAsync(string command, int reserved)
        {
            return Task.Run<int>(() =>
            {
                char* commandChar = stringToChar(command);
        ...
        /// <summary>
        /// [Async] Sets a new protocol specific parameter. Please see the documentation of the protocols before setting any parameter.
        /// </summary>
        /// <param name="deviceId">Id of device to change</param>
        /// <param name="name">The name of the parameter to change</param>
        /// <param name="value">The new value for the parameter</param>
        /// <returns>Success or error code</returns>
        public static unsafe Task<bool> tdSetDeviceParameterAsync(int deviceId, string name, string value)
        {
            return Task.Run<bool>(() =>
            {
                char* nameChar = stringToChar(name);
        ...
        /// <summary>
        ///  [Async] Sets a new model for a device. Which model to set depends on the current protocol.
        /// </summary>
        /// <param name="deviceId">Id of device to change</param>
        /// <param name="model">The new model</param>
        /// <returns>Success or error code</returns>
        public static unsafe Task<bool> tdSetModelAsync(int deviceId, string model)
        {
            return Task.Run<bool>(() =>
            {
                char* modelChar = stringToChar(model);
        ...
        /// <summary>
        /// [Async] Sets a new name for a device.
        /// </summary>
        /// <param name="deviceId">Id of device to change</param>
        /// <param name="name">The new name</param>
        /// <returns>Success or error code</returns>
        public static unsafe Task<bool> tdSetNameAsync(int deviceId, string name)
        {
            return Task.Run<bool>(() =>
            {
                char* nameChar = stringToChar(name);
        ...
        /// <summary>
        /// [Async] This changes the current protocol used by a device. After changing the protocol, setting new parameters is required.
        /// </summary>
        /// <param name="deviceId">Id of device to change</param>
        /// <param name="protocol">The new protocol to use</param>
        /// <returns>Success or error code</returns>
        public static unsafe Task<bool> tdSetProtocolAsync(int deviceId, string protocol)
        {
            return Task.Run<bool>(() =>
            {
                char* protocolChar = stringToChar(protocol);
        ...
        /// <summary>
        /// [Async] Send "stop" command to device.
        /// Make sure the device supports this by calling tdMethods() before any calls to this function.
        /// </summary>
        /// <param name="deviceId">The device id to stop</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdStopAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdStop(deviceId);
        ...
        /// <summary>
        /// [Async] Turns a device on.
        /// Make sure the device supports this by calling tdMethods() before any calls to this function.
        /// </summary>
        /// <param name="deviceId">Id of device to turn on</param>
        /// <returns>Success or error code</returns>
        public static Task<int> tdTurnOnAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdTurnOn(deviceId);
        ...
        /// <summary>
         /// [Async] Turns a device off.
         /// Make sure the device supports this by calling tdMethods() before any calls to this function.
         /// </summary>
         /// <param name="deviceId">Id of device to turn off</param>
         /// <returns>Success or error code</returns>
        public static Task<int> tdTurnOffAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdTurnOff(deviceId);
		...
		{
			this.deviceEventList.Remove(eventId);
			if (this.deviceEventList.Count == 0)
			{
				//no more events in list
				UnmanagedImport.tdUnregisterCallback(this.registeredEventFunctionId);
				this.callbackFunctionReferenceList.Remove(this.registeredEventFunctionId);
				/*
			...
			this.deviceChangeEventList.Remove(eventId);
			if (this.deviceChangeEventList.Count == 0)
			{
				//no more events in list
				UnmanagedImport.tdUnregisterCallback(this.registeredDeviceChangeEventFunctionId);
				this.callbackFunctionReferenceList.Remove(this.registeredDeviceChangeEventFunctionId);
			}

			this.rawListenerList.Remove(eventId);
			if (this.rawListenerList.Count == 0)
			{
				//no more events in list
				UnmanagedImport.tdUnregisterCallback(this.registeredRawListenerFunctionId);
				this.callbackFunctionReferenceList.Remove(this.registeredRawListenerFunctionId);
			}
		}
        ...
        /// <summary>
		/// [Async] Send "up" command to device.
		/// Make sure the device supports this by calling tdMethods() before any calls to this function.
		/// </summary>
		/// <param name="deviceId">The device id to send the command to</param>
		/// <returns>Success or error code</returns>
		public static Task<int> tdUpAsync(int deviceId)
        {
            return Task.Run<int>(() =>
            {
                return UnmanagedImport.tdUp(deviceId);
		...
		{
			foreach (DeviceEventFunctionContext deviceEventFuncContext in this.deviceEventList.Values)
			{
		...
		{
			foreach (DeviceChangeEventFunctionContext deviceChangeEventFuncContext in this.deviceChangeEventList.Values)
			{
		...
		{
			foreach (RawEventFunctionContext rawListeningFuncContext in this.rawListenerList.Values)
			{
